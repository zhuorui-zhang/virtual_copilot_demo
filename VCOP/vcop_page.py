...
def page(args):
    ...
    with st.container(border=True):
        ...
        with st.form(key='user_input_form'):
            submit_button = st.form_submit_button(label='Start Recognizing from uploaded file...')
            if submit_button:
        ...
        # 控制按钮
        record_button = st.button("START/END")
        utils.ChangeButtonColour('START/END', 'black', '#ACF4A2')
        if record_button:
            if st.session_state.recording:
                utils.ChangeButtonColour('START/END', 'black', '#ACF4A2')
                stop_recording()
            else:
                utils.ChangeButtonColour('START/END', 'black', '#FC5252')
                start_recording()
        
        tmp_record_button = st.button("开始识别口头输入...")
