    ...
    for i in range(len(bboxes)):
        for j in range(i+1,len(bboxes)):
            if calculate_IoU(bboxes[i], bboxes[j])>th:
                ...
                cb_list.append([min(bboxes[i][0], bboxes[j][0]),min(bboxes[i][1], bboxes[j][1]),max(bboxes[i][2], bboxes[j][2]),max(bboxes[i][3], bboxes[j][3])])
    if len(cb_list)>0:
...
def Morph_exam(test_img,binary):
    ...
    for i in range(len(filtered_boxes_cp)):
        ...
        cv2.rectangle(test_img_cp1, (rect[0], rect[1]), (rect[2], rect[3]), (255, 0, 0), 4)
        cv2.putText(test_img_cp1, str(n), (rect[0], rect[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        cv2.imshow('img', test_img_cp1)
        n = n+1
...
def red_bnd_extract(img, word_img):
    ...
    cv2.namedWindow("red_boxes",0)
    for j in range(len(red_boxes)):
        rect = red_boxes[j]
        cv2.rectangle(cv_img, (rect[0], rect[1]), (rect[2], rect[3]), (255, 0, 0), 4)
        cv2.putText(cv_img, str(j), (rect[0], rect[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        cv2.imshow('red_boxes', cv_img)
    return red_boxes
...
def amber_bnd_extract(img,word_img):
    ...
    cv2.namedWindow("amber_region",0)
    cv2.imshow("amber_region",amber_region)
    ...
    t, amber_binary1 = cv2.threshold(gray,0, 255, cv2.THRESH_BINARY |
                                     cv2.THRESH_OTSU)
    cv2.namedWindow("amber_binary1",0)
    cv2.imshow("amber_binary1",amber_binary1)
    ...
    cv2.namedWindow("amber_binary",0)
    cv2.imshow("amber_binary",amber_binary)

    amber_boxes = Morph_exam(amber_region, amber_binary)

    cv2.namedWindow("amber_boxes", 0)
    ...
    for j in range(len(amber_boxes)):
        rect = amber_boxes[j]
        cv2.rectangle(cv_img, (rect[0], rect[1]), (rect[2], rect[3]), (255, 0, 0), 4)
        cv2.putText(cv_img, str(j), (rect[0], rect[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        cv2.imshow('amber_boxes', cv_img)
    return amber_boxes
...
def rectangles_near(rect1, rect2,th=0.2):
    ...
    if th==0:
        ...
        if (x0_2 >= x0_1 and x0_2<=x1_1 ) and (y0_2>=y0_1 and y0_2<=y1_1):
            ...
            return 1
        elif (x0_2 >= x0_1 and x0_2<=x1_1) and abs(y1_1-y0_2) < min(abs(th*(y1_1-y0_1)),abs(th*(y1_2-y0_2))):
            return 1
        elif (x0_1 >= x0_2 and x0_1<=x1_2) and abs(y1_2-y0_1) < min(abs(th*(y1_1-y0_1)),abs(th*(y1_2-y0_2))):
            return 1
...
def get_boxes(filename,img):
    ...
    if len(red_boxes)>0:
        ...
        if min([red_box[3] for red_box in red_boxes] ) > h/2 and max([red_box[3] - red_box[1] for red_box in red_boxes]) > 140:
            row_r = 3*row_r
        if min([red_box[3] for red_box in red_boxes] ) < h/2 and max([red_box[3] - red_box[1] for red_box in red_boxes]) > 95:
            row_r = 3*row_r
        red_conf=1
        ...
        right=max(left+12*sum([red_box[3] -red_box[1] for red_box in red_boxes])/max(len(red_boxes),row_r),max([red_box[2] for red_box in red_boxes]))
    ...
    if len(remain_bnd_ls)>0:
        ...
        roi_value={"xmin":max(0,int(0.95*(min_xmin)/(1))-8),
                   ...
                   "ymax":min(h, int((min(min_ymin+min(8*sum(h_ls)/len(h_ls),2*h/7),max_ymax))/(1))+8)}

        cv2.namedWindow("boxes_img",0)
        for i in range(len(remain_bnd_ls)):
            ...
            warning = {"name": "warning", "bndbox": {"xmin":max(0,int(0.5*rect["xmin"]+0.5*min_xmin)-5), "ymin":rect["ymin"]-5, "xmax":rect["xmax"], "ymax":rect["ymax"]}}
            warning_ls.append(warning)
        cv2.rectangle(cv_img, (roi_value["xmin"], roi_value["ymin"]), (roi_value["xmax"], roi_value["ymax"]), (255, 255, 0), 4)
        cv2.imwrite("tmp1/" + filename, cv_img)
        cv2.imshow('boxes_img', cv_img)
        #cv2.waitKey(0)
