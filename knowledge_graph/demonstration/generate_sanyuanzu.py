...
def connectNeo4j_fun():
    ...
    # webbrowser.open("http://localhost:7474/")  #生成三元组不打开浏览器后台运行就可以
...
def creat_crewman(df_crewman,defined_nodes,graph, matcher,sanyuanzu):
    ...
    return defined_nodes,sanyuanzu

def creat_feedback(df_feedback,defined_nodes,graph, matcher,sanyuanzu):
    for index, row in df_feedback.iterrows():#定义feedback
        ...
        sanyuanzu.append([X,relation,Y])

        graph.create(relationship)
    return defined_nodes,sanyuanzu

def creat_action(df_action,defined_nodes,graph, matcher,sanyuanzu):
    ...
    for index, row in df_action.iterrows():
        ...
        sanyuanzu.append([X,relationship,Y])
        graph.create(relationship)
    return defined_nodes,sanyuanzu

def creat_event(df_event,defined_nodes,graph, matcher,sanyuanzu):
...
def creat_condition(df_condition,defined_nodes,graph, matcher,sanyuanzu):
...
def fun_2sanyuanzu(file,output_sanyuanzu_path):
    connectNeo4j_fun()
    usr = 'neo4j'
    key = '180823'
    ...
    sanyuanzu=[]
    #定义crewman
    defined_nodes,sanyuanzu=creat_crewman(df_crewman,defined_nodes,graph, matcher,sanyuanzu)
    #定义feedback
    defined_nodes,sanyuanzu=creat_feedback(df_feedback,defined_nodes,graph, matcher,sanyuanzu)
    #定义action
    defined_nodes,sanyuanzu=creat_action(df_action, defined_nodes, graph, matcher,sanyuanzu)
    #定义event
    defined_nodes,sanyuanzu=creat_event(df_event, defined_nodes, graph, matcher,sanyuanzu)
    # 定义condition
    defined_nodes,sanyuanzu= creat_condition(df_condition, defined_nodes, graph, matcher,sanyuanzu)
    df_sanyuanzu=pd.DataFrame(sanyuanzu)
    df_sanyuanzu.to_excel(output_sanyuanzu_path)
    print('三元组生成成功')


if __name__=="__main__":
    file=r'90-FMS Engine Failure Procedures/20240502104237-知识提取结果.xlsx'
    output_sanyuanzu_path='三元组.xlsx'
    fun_2sanyuanzu(file, output_sanyuanzu_path)